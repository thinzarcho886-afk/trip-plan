  ...
  <v-container class="my-6">
    ...
    <v-card class="elevation-1">
      <v-table density="compact" class="text-no-wrap">
        ...
        <tbody>
          <tr v-if="loading">
            <td colspan="19" class="text-center py-4">{{ t('Loading history...') }}</td>
          </tr>
          <tr v-else-if="bookingList.length === 0">
            <td colspan="19" class="text-center py-4">{{ t('No booking history found.') }}</td>
          </tr>
          <tr v-for="item in bookingList" :key="item.id">
            ...
            <td>{{ item.departureDate ? item.departureDate.split('T')[0] : '-' }}</td>
            ...
            <!-- 1. ListStatus Component သို့ ပြောင်းလဲထားပါသည် -->
            <td>
              <ListStatus :status="item.status" />
            </td>

            <!-- 2. ListDateTime Component သို့ ပြောင်းလဲထားပါသည် -->
            <td>
              <ListDateTime :milliseconds="item.updatedDateInMilliSeconds" />
            </td>
...
// Component များကို Import ခေါ်ယူထားပါသည်
