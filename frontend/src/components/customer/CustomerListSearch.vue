...
const searchParams = ref<CustomerListParams>(CustomerListParamsModel());
...
const onReset = () => {
  searchParams.value = CustomerListParamsModel();
  onSearch();
};
...
onMounted(() => {
  const { routeFilter, isFiltered } = useRouteFilter<CustomerListParams>(
    searchParams.value,
    route.query
  );
