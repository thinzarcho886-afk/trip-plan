...
import { PaymentMethodListParams, PaymentMethodListParamsModel } from '../../models/PaymentMethodModel';
...
const searchParams = ref<PaymentMethodListParams>(PaymentMethodListParamsModel());
...
const onReset = () => {
  searchParams.value = PaymentMethodListParamsModel();
  onSearch();
};
...
onMounted(() => {
  const { routeFilter, isFiltered } = useRouteFilter<PaymentMethodListParams>(
    searchParams.value,
    route.query
  );
