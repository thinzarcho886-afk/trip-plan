...
import CityPicker from '../city/CityPicker.vue';
import { useI18n } from 'vue-i18n';
import { useAuthStore } from '../../store/auth';
import { Role } from '../../constants/Role';
import EnumPicker from '../common/EnumPicker.vue';
import { CityListParams, CityListParamsModel } from '../../models/CityModel';
...
const searchParams = ref<CityListParams>(CityListParamsModel());

const onSearch = async () => {
  if (authStore.userRole != Role.SYSADMIN) {
    searchParams.value.cityName = authStore.user.cityName;
  }
...
const onReset = () => {
  searchParams.value = CityListParamsModel();
  onSearch();
};
...
onMounted(() => {
  const { routeFilter, isFiltered } = useRouteFilter<CityListParams>(
    searchParams.value,
    route.query
  );
