...
import com.cbk.devconstruction.repository.RoleRepository;
...
import com.cbk.devconstruction.specification.UserSpecs;
...
//    }
	...
	public List<UserDTO> getOwnerByName(String ownerName) {
		List<UserDTO> ownerDTOList = new ArrayList<>();
		Optional<List<User>> ownerOpt = null;
		ownerOpt = userRepository.findByOwnerName(ownerName);
		if (ownerOpt.isPresent()) {
			for (User owner : ownerOpt.get()) {
				UserDTO ownerList = new UserDTO(owner);

				ownerDTOList.add(ownerList);
			}
		}
	...
	public List<UserDTO> getOwnerByPhone(String phone) {
		List<UserDTO> ownerDTOList = new ArrayList<>();
		Optional<List<User>> ownerOpt = null;
		ownerOpt = userRepository.findByOwnerPhone(phone);
		if (ownerOpt.isPresent()) {
			for (User owner : ownerOpt.get()) {
				UserDTO ownerList = new UserDTO(owner);
	...
	public List<UserDTO> getOwnerByAddress(String address) {
		List<UserDTO> ownerDTOList = new ArrayList<>();
		Optional<List<User>> ownerOpt = null;
		ownerOpt = userRepository.findByOwnerAddress(address);
		if (ownerOpt.isPresent()) {
			for (User owner : ownerOpt.get()) {
				UserDTO ownerList = new UserDTO(owner);
	...
	public List<UserDTO> getStudentByName(String studentName) {
		List<UserDTO> studentDTOList = new ArrayList<>();
		Optional<List<User>> studentOpt = null;
		studentOpt = userRepository.findByStudentName(studentName);
		if (studentOpt.isPresent()) {
			for (User student : studentOpt.get()) {
				UserDTO studentList = new UserDTO(student);

				studentDTOList.add(studentList);
			}
		}
	...
	public List<UserDTO> getStudentByPhone(String phone) {
		List<UserDTO> studentDTOList = new ArrayList<>();
		Optional<List<User>> studentOpt = null;
		studentOpt = userRepository.findByStudentPhone(phone);
		if (studentOpt.isPresent()) {
			for (User student : studentOpt.get()) {
				UserDTO studentList = new UserDTO(student);
	...
	public List<UserDTO> getStudentByAddress(String address) {
		List<UserDTO> studentDTOList = new ArrayList<>();
		Optional<List<User>> studentOpt = null;
		studentOpt = userRepository.findByStudentAddress(address);
		if (studentOpt.isPresent()) {
			for (User student : studentOpt.get()) {
				UserDTO studentList = new UserDTO(student);
