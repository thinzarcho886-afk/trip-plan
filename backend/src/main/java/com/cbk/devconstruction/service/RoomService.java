...
public class RoomService {
	...
	public PageableDTO getRooms(String roomNo,String roomTypeName,RoomStatus roomStatus, Status status, Boolean has,Gender gender, String hostelName,Long hostelId, String ownerName, Pageable pageable) {
		...
		Specification<Room> roomSpecs=RoomSpecs.getByFilter(roomNo, roomTypeName, roomStatus, status, has, gender, hostelName,hostelId, ownerName);
	...
	public RoomDTO save(@Valid RoomDTO dto, boolean isUpdate) throws IOException {
		Room room=new Room();
		if(isUpdate)room=CommonUtil.checkValidById(dto.getId(), roomRepository);
		
		if(dto.getHostelId() != null) {
			Hostel hostel=hostelRepository.findById(dto.getHostelId()).orElseThrow(() -> new EntityNotFoundException("Hostel Not found with id:"+dto.getHostelId()));
		...
		if (!isUpdate) {
			room.setRoomImage(
					NginxUtil.saveImage(dto.getRoomImageUrl(), "room_image"));
		} else {
			room.setRoomImage(
					NginxUtil.updateImage(dto.getRoomImageUrl(), room.getRoomImage(),
							"room_image", StringUtils.isEmpty(dto.getRoomImage())));
		}
		
		}
		room.setRoomType(roomTypeService.checkValidRoomType(dto.getRoomTypeId()));
